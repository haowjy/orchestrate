...
For each issue found, create a cleanup file in `{{SLICES_DIR}}/cleanup-NNN.md` (if `{{SLICES_DIR}}` is available) or `scratch/review/cleanup-NNN.md` under the scope root (for interactive use), describing:
